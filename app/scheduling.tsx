...
const SchedulingScreen = () => {
  ...
  const matchData = useSelector((state) => state?.app?.singleMatchData);

  const router = useRouter();

  const { addItem } = useLocalDatabase();
  const handleSelectDay = (data: any) => {
    setSelectedDay(data);
  };

  const handleSchedulMatch = () => {
    let id = uuid.v4();
    ...
    console.log(newSchedule, "jhsdfjsdf");
  };
    ...
    <View style={styles.screen}>
      <View style={styles.scheduleBox}>
        ...
        <View style={{ paddingTop: "10%" }}>
          ...
          <TouchableOpacity onPress={handleSchedulMatch} style={styles.button}>
            <Text
              style={{
                textAlign: "center",
